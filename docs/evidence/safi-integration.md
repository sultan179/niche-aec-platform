...
## 2026-09-22 — Root cause of segment splitting found: SAFI auto-inserts "Rigid Member" links

- **FACT (source: SAFI Member Attributes dialog, direct observation, live inspection of the beam run at the two unexplained split points from 2026-09-18):** the element bridging the gap is a **"Rigid Member"** — `Member Type: Offset/Dummy`, `Section: Rigid Member [Rigid]`, length **93.4mm**, connecting `Joint 22` to `Joint 92`. It's clearly visible in the SAFI 3D view (a short diagonal line) once you know to look for it, but wasn't detectable from the SDNF export.
- **FACT (source: Sultan, direct comparison):** opening the drafter's original native SAFI file gives exactly-coincident joints (clean connection, no rigid member). **Importing the same geometry via IFC** introduces small coordinate gaps (rounding through the Revit→IFC→SAFI pipeline), and SAFI's connectivity tolerance logic bridges those gaps with an auto-inserted Rigid Member instead of merging the joints.
- **Why our SDNF-based proximity search (`find_junctions.py`) found nothing:** SDNF is a **steel detailing** format — it only exports real, fabricatable physical members. A Rigid Member (`Offset/Dummy` type) isn't real steel, so **SAFI correctly excludes it from the SDNF export**. The bridging element exists live in SAFI's model but is invisible to any tool reading SDNF alone.
- **Conclusion:** the "unexplained" splits from 2026-09-18 are not a real structural connection, and not an unrelated internal SAFI meshing behavior either — they're a **byproduct of coordinate imprecision introduced by the IFC import pipeline**, resolved automatically (and silently) by SAFI's own tolerance logic. This fully answers the question drafted for the drafter — resolved by direct inspection, no drafter input needed.
- **Implication (RECOMMENDATION) for the matcher:** this data can never be reconstructed from SDNF alone, since the bridging Rigid Members simply aren't in that file. For future 1:N/chain-merge work, the practical fix isn't detecting these rigid links directly — it's widening the *outer* endpoint tolerance used when matching a Revit beam (or run of beams) against its corresponding SAFI segment chain, since the combined chain's true outer endpoints can be offset from Revit's by up to a Rigid-Member's-worth of gap (bounded by SAFI's ~200mm tolerance). This also explains the joint-count growth seen in every import report (e.g. 123→144 on the Cleaned file) — each Rigid Member adds its own 2 joints.
