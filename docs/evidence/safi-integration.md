...
- **FACT (verified via simulation, source: scratch test script run against the real `baseline.json`, since no second real change-containing export exists yet):** took the saved baseline, mutated 4 real matched elements in a copy of it (Revit section only, SAFI section only, both, and one element removed entirely), plus added one fake new element. All 5 classified correctly (`changed in Revit only`, `changed in SAFI only`, `conflict - changed in both`, `missing`, `new`), and all 73 untouched elements stayed `unchanged` — zero false positives. This is a simulation on real matched data, not synthetic/fabricated elements, but it is **not** a second real-world change (no second export of this project with an actual edit exists yet) — that's still worth doing once available.
