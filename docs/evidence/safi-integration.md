...
- **Still untested:** the diff has only been run against the no-change case (same file vs. its own baseline). It has not yet been run against a baseline where a real (or simulated) section/material change exists — the actual case the tool needs to get right.
