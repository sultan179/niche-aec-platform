...
- **FACT (2026-09-24) — code review caught two real bugs, both fixed and verified:** (1) `explain_unmatched`'s "lost the tie-break" message claimed the winning candidate "matched a closer element instead" without checking that was true — reproduced with a real 4-element scenario where the candidate ends up completely unmatched too, not stolen by anyone. Fixed to state only what's verifiable. (2) the `"ambiguous"` status was overwriting the section-check result entirely, so an ambiguous match that was *also* a genuine section mismatch would hide the mismatch — the one thing the drafter actually cares about. Fixed: status always reflects the section check; ambiguity goes in the `reason` column instead.
- **FACT (2026-09-24) — a second round of review (after wiring ambiguity into `three_way_diff.py`) found two more real issues, both fixed:** (1) an old baseline saved before the ambiguous feature existed would make every currently-ambiguous element look `newly_ambiguous=True`, even with nothing actually changed, since the old baseline never recorded ambiguity to compare against — fixed by re-saving `baseline.json` now that it carries the new column. (2) `three_way_diff.py` was detecting ambiguity by string-matching `reason.startswith("ambiguous")` — fragile, since it silently breaks if the message text ever changes. Fixed: `report.py` now outputs a real boolean `ambiguous` column; `three_way_diff.py` reads that directly. Also: added `test_three_way_diff.py` (4 tests, closing the last untested module), extracted a shared `MATCH_TOL_M` constant (was hardcoded `0.5` in 4 places), and minor cleanup (dead comment references, `desktop.ini` added to `.gitignore`). All 44 tests pass.
- **Open, deliberately not fixed yet (design judgment call, not a bug):** `find_ambiguous()` flags any pair with 2+ same-category candidates within tolerance, even if one is dramatically closer (e.g. 0mm vs 480mm). Zero ambiguous matches found in real data so far, so not yet known to cause noise — worth tightening (e.g. requiring the runner-up to be within some margin of the winner, not just within tolerance) if it does.
...
- `find_ambiguous()` doesn't account for how much closer the winning candidate was — a pair at 0mm with a runner-up at 480mm still counts as ambiguous. Not yet known to cause real noise (0 ambiguous matches so far), worth tightening if it does.
...
7. ~~Add unit tests for `three_way_diff.py`~~ — done, all 4 modules in the pipeline now have tests (44 total).
