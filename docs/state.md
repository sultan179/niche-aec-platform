...
- **FACT (2026-09-23) — verified end-to-end on a genuine real-world change:** changed one real Girder's section in the actual Revit project (`W18X40` → `W21X44`), re-exported IFC, ran the full pipeline against the existing (untouched) SAFI export and the saved baseline. Result: **77/78 unchanged, exactly 1 correctly flagged `changed in Revit only`** with the right before/after values. This is on top of the earlier same-data (78/78 unchanged) and mutated-data-simulation (5/5 correct, 0 false positives) checks — the three-way diff mechanism is now proven at all three levels, not just simulation.
...
- The real-world edit test only exercised one path (changed in Revit only). The other four (changed in SAFI only, conflict, new, missing) are still simulation-only on real data, not yet confirmed with an actual SAFI-side or combined edit.
...
1. Test the three-way diff's remaining paths (changed-in-SAFI-only, conflict, new, missing) against real edits, not just simulated ones — only changed-in-Revit-only has been confirmed on a genuine real-world change so far.
