...
- Three-way diff proven on the no-change case and on a simulated change (mutated real data) — still not tested against a genuine second real-world export with an actual edit in it.
...
1. Test the three-way diff against a genuine second real-world export with an actual change in it (simulated version already passes; real one not yet available).
