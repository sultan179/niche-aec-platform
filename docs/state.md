...
- Three-way diff only proven on the no-change case (same file vs. its own baseline) — not yet tested against a real or simulated change.
...
1. Test the three-way diff against a real or simulated change (section/material edited on one side only) — only the no-change case is proven so far.
