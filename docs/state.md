...
- **Current objective:** 1:N/N:N chain matching built and verified. Next: grow SectionMapping with more real data; decide G1 criteria with the drafter.
...
5. Build a MaterialMapping dictionary (same pattern as SectionMapping) once real material pairs are confirmed — formalizes the same manual sync the drafter already does for materials.
