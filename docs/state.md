...
- `match.py` (geometry matching, chain matching) and `three_way_diff.py` still have no unit tests — only `section_mapping.py`'s normalization logic is covered so far.
...
7. Add unit tests for `match.py` (geometry matching, chain matching) and `three_way_diff.py` — `section_mapping.py` is covered now, those two aren't.
