...
- **FACT (2026-09-22, drafter):** SAFI is the sizing/material source of truth — decisions finalize during analysis in SAFI, and the drafter manually syncs Revit to match afterward, **for both sections and materials**. Confirms the SectionMapping dictionary formalizes an existing manual task rather than solving a hypothetical one, and that there's currently no automated check that the sync happened correctly — exactly the gap the reconciliation report fills.
...
- **FACT (2026-09-23) — verified end-to-end on genuine real-world changes, both directions:** changed one real Girder's section in Revit (`W18X40` → `W21X44`, Revit untouched on the SAFI side) — result: 77/78 unchanged, 1 correctly flagged `changed in Revit only`. Then changed one real member's section in SAFI (`W460x60` → `W530x66`, SAFI untouched on the Revit side) — result: 77/78 unchanged, 1 correctly flagged `changed in SAFI only`, right values on both sides. Two of the five diff paths now proven on genuine edits, on top of the earlier same-data (78/78 unchanged) and mutated-data-simulation (5/5 correct, 0 false positives) checks. `conflict`, `new`, and `missing` remain simulation-only.
...
- Real-world edit tests have covered 2 of 5 paths (changed in Revit only, changed in SAFI only). `conflict`, `new`, and `missing` are still simulation-only on real data, not yet confirmed with an actual edit.
...
1. Test the three-way diff's remaining paths (conflict, new, missing) against real edits, not just simulated ones — changed-in-Revit-only and changed-in-SAFI-only are both confirmed on genuine real-world changes now.
...
6. Build a MaterialMapping dictionary (same pattern as SectionMapping) once real material pairs are confirmed — formalizes the same manual sync the drafter already does for materials.
